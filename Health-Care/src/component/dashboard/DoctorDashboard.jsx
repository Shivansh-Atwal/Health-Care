...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch(`${import.meta.env.VITE_URL}appointments/requests`, { headers: { Authorization: `Bearer ${token}` } }),
      fetch(`${import.meta.env.VITE_URL}appointments/doctor/all`, { headers: { Authorization: `Bearer ${token}` } }),
      fetch(`${import.meta.env.VITE_URL}reports/my-reports`, { headers: { Authorization: `Bearer ${token}` } })
    ])
  ...
  useEffect(() => {
    if (view === 'attendance' && user?.role === 'doctor' && user._id && token) {
      ...
      fetch(`${import.meta.env.VITE_URL}auth/doctors/attendance/${user._id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
        .then(res => res.json())
  ...
  const markAttendance = async () => {
    ...
    setAttendanceMsg('');
    try {
      const res = await fetch(`${import.meta.env.VITE_URL}auth/doctors/attendance`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAction = async (id, action, scheduledTime) => {
    let url = `${import.meta.env.VITE_URL}appointments/${id}/${action}`;
    let options = {
      method: 'POST',
  ...
  const handleDeleteReport = async (id) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_URL}reports/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
