  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      const [attendanceRes, doctorsRes] = await Promise.all([
        fetch(`${import.meta.env.VITE_URL}auth/doctors/attendance/all`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${import.meta.env.VITE_URL}auth/doctors`, { headers: { Authorization: `Bearer ${token}` } })
      ]);
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        const [feedbackRes, reportsRes, appointmentsRes, statsRes] = await Promise.all([
          fetch(`${import.meta.env.VITE_URL}feedback`, { headers: { Authorization: `Bearer ${token}` } }),
          fetch(`${import.meta.env.VITE_URL}reports`, { headers: { Authorization: `Bearer ${token}` } }),
          fetch(`${import.meta.env.VITE_URL}appointments/all`, { headers: { Authorization: `Bearer ${token}` } }),
          fetch(`${import.meta.env.VITE_URL}global-stats`)
        ]);
